...
function viewStudentStats(id, toggle){
	...
	if(toggle == 0){
		if(id == default_id){
			// If id is default hide everything
			document.getElementById(id).style.visibility = "hidden";
			document.getElementById(id).style.height = 0;
			document.getElementById(old_id).style.visibility = "hidden";
			document.getElementById(old_id).style.height = 0;
		}
		else{
			// if id not default, only hide current id
			document.getElementById(id).style.visibility = "hidden";
			document.getElementById(id).style.height = 0;
		}
	}
...
function switchPlayers(playerNum){
	...
	// Avg. recieving yards
	document.getElementById('avg_rec_yards').innerHTML = ((players[playerNum].rushing_yards)/(players[playerNum].games_played)).toFixed(3);
}
